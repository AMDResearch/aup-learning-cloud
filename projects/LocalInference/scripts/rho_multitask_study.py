...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--generations", type=int, default=2, choices=(2, 3, 4))
...
def selected_candidate(root: Path, frontier: dict) -> tuple[str, Path]:
    candidate_id = max(
        frontier["candidates"],
        key=lambda current: (
            sum(frontier["candidates"][current]["scores"].values()),
            current,
        ),
    )
    worktree = root / ".helix" / "worktrees" / candidate_id
    return candidate_id, worktree if worktree.is_dir() else root
...
def main() -> int:
    ...
    try:
        ...
        best_id, best_root = selected_candidate(root, frontier)
