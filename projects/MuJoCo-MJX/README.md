...
Labs (Image 1):
- Module A — MuJoCo Foundations (MJX01–03)
...
- Module B — MJX & large-scale parallel simulation (MJX04–05)
...
stack they run with `impl="jax"`, `num_envs=256`, and highest matmul precision;
MJX09 additionally lowers the learning rate and clips gradients to avoid NaNs.

Note: Module A is pure MuJoCo (no RL framework needed), so it runs natively on
