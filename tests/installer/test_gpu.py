...
def test_supported_accelerators_are_exactly_curated() -> None:
    assert CURATED_ACCELERATOR_KEYS == ("strix-halo", "9060", "9060xt", "9070", "9070xt", "r9700")


@pytest.mark.parametrize("product", ["AMD_Radeon_780M_Graphics", "AMD_Radeon_890M_Graphics", "AMD_Radeon_RX_9600_GRE"])
def test_removed_products_fail(product: str) -> None:
    with pytest.raises(InstallerError, match="Unsupported AMD GPU product"):
        sku_for_product_name(product)


@pytest.mark.parametrize("value", ["gfx1100", "gfx1150", "gfx120x", "rdna4", "dgpu"])
def test_removed_profiles_and_fallbacks_fail(value: str) -> None:
    with pytest.raises(InstallerError):
        resolve_gpu_config(value)


def test_unique_detected_gfx_resolves_to_profile() -> None:
    row = resolve_gpu_config("gfx1151")
    assert (row.accelerator_key, row.image_profile) == ("strix-halo", "gfx1151")
