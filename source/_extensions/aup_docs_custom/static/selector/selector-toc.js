...
function getUniqueGroups(groups) {
  ...
  return groups.filter((group) => {
    const headingSpan = group.querySelector(".rocm-docs-selector-group-heading-text");
    const headingText = headingSpan ? headingSpan.textContent.trim() : "(Unnamed Selector)";
...
export function updateTOC2OptionsList() {
  ...
  let visibleGroups = Array.from(document.querySelectorAll(GROUP_QUERY)).filter(isVisible);
...
function initTOC2ContentsList() {
  ...
  tocContentsList.querySelectorAll(`li.toc-entry.${TOC_ITEM_CLASS}`).forEach((node) => node.remove());
  ...
  headings.forEach((h) => {
    ...
    const li = document.createElement("li");
    li.className = `nav-item toc-entry toc-${h.tagName.toLowerCase()} ${TOC_ITEM_CLASS}`;
    ...
    const firstTextNode = clone.childNodes.length > 0 ? clone.childNodes[0].textContent : "";
    ...
    else tocContentsList.appendChild(li);
    lastLiByLevel[level] = li;
    for (let deeper = level + 1; deeper <= 6; deeper += 1) delete lastLiByLevel[deeper];
  });
  contentsTocInitialized = true;
}
...
export function updateTOC2ContentsList() {
  ...
  if (!contentsTocInitialized) initTOC2ContentsList();
  tocContentsList.querySelectorAll(`li.toc-entry.${TOC_ITEM_CLASS}`).forEach((li) => {
